...
// INITIAL MAP SETUP
// ===============================
L.tileLayer("tiles/{z}/{x}/{y}.png", { // local tiles folder
    maxZoom: 18
}).addTo(map);
...
function loadCountryMarkers() {
    ...
    currentCountryInfo.cities.forEach(city => {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
            L.marker([lat, lng])
                .bindPopup(`${city.name}<br>Population: ${city.population ? numeral(city.population).format('0,0') : "N/A"}`)
                .addTo(cityLayer);
        }
    });
}
...
L.easyButton("fa-cloud", () => {
    if (!currentCountryInfo || !currentCountryInfo.capital) return alert("Select a country first.");
...
function fetchWeather(city) {
    ...
    fetch(weatherUrl)
        .then(r => r.json())
        .then(forecast => {
            if (!forecast.list || !forecast.list.length) return alert("No weather data available.");
            ...
            showWeatherModal({
                ...
                forecast: forecast.list
                    ...
                    .slice(0, 5)
                    .map(f => ({
                        day: dateFns.format(new Date(f.dt_txt), "EEE"),
...
function fetchPublicHolidays(countryCode) {
    ...
    $.getJSON(`php/getPublicHolidays.php?country=${countryCode}`, function(data) {
        ...
        data.forEach(holiday => {
            list.append(`<li class='list-group-item'>${dateFns.format(new Date(holiday.date), "PPP")} - ${holiday.localName}</li>`);
        });
...
// ===============================
// AUTO-SELECT COUNTRY USING GEOLOCATION
// ===============================
        ...
        function (position) {
            $.ajax({
                ...
                success: function (result) {
                    ...
                    } else console.warn('Country lookup failed');
                },
                error: function () { console.error('AJAX error getting country code'); }
            });
        },
        function () { console.warn('Geolocation permission denied'); }
    );
}
