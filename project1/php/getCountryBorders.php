...
// Load GeoJSON file
$json = file_get_contents(__DIR__ . '/../data/countryBorders.geo.json');
...
// Convert ISO2 to uppercase
$code = strtoupper($code);

// Search GeoJSON for matching country
$feature = null;
foreach ($data['features'] as $f) {
    // Some GeoJSON use ISO_A2, others ISO_A3
    $isoA2 = $f['properties']['ISO_A2'] ?? '';
    $isoA3 = $f['properties']['ISO_A3'] ?? '';

    if ($code === $isoA2 || $code === $isoA3 || ($iso3to2[$code] ?? '') === $isoA2) {
        $feature = $f;
        break;
    }
}
...
if ($feature) {
    ...
    echo json_encode(["error" => "Country border not found for code: {$code}"]);
}
?>
